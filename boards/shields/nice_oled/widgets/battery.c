...
static void draw_level(lv_layer_t *layer, const struct status_state *state) {
    ...
    lv_area_t area = {CONFIG_NICE_OLED_WIDGET_BATTERY_CUSTOM_X, CONFIG_NICE_OLED_WIDGET_BATTERY_CUSTOM_Y,
                      CONFIG_NICE_OLED_WIDGET_BATTERY_CUSTOM_X + 42 - 1, CONFIG_NICE_OLED_WIDGET_BATTERY_CUSTOM_Y + 200};
...
static void draw_charging_level(lv_layer_t *layer, const struct status_state *state) {
    ...
    sprintf(text, "%i", state->battery);
    label_right_dsc.text = text;
    lv_area_t text_area = {CONFIG_NICE_OLED_WIDGET_BATTERY_CUSTOM_X, CONFIG_NICE_OLED_WIDGET_BATTERY_CUSTOM_Y,
                           CONFIG_NICE_OLED_WIDGET_BATTERY_CUSTOM_X + 35 - 1, CONFIG_NICE_OLED_WIDGET_BATTERY_CUSTOM_Y + 200};
    lv_draw_label(layer, &label_right_dsc, &text_area);

#if IS_ENABLED(CONFIG_NICE_EPAPER_ON)
    int bolt_x = CONFIG_NICE_OLED_WIDGET_BATTERY_CUSTOM_X + 36;
    int bolt_y = CONFIG_NICE_OLED_WIDGET_BATTERY_CUSTOM_Y + 2;
#else
    int bolt_x = CONFIG_NICE_OLED_WIDGET_BATTERY_CUSTOM_X + 25;
    int bolt_y = CONFIG_NICE_OLED_WIDGET_BATTERY_CUSTOM_Y;
#endif // CONFIG_NICE_EPAPER_ON
    lv_area_t img_area = {bolt_x, bolt_y, bolt_x + bolt.header.w - 1, bolt_y + bolt.header.h - 1};
...
void draw_battery_status(lv_layer_t *layer, const struct status_state *state) {
    ...
    lv_area_t bat_area = {0, 19, 24, 200};
    lv_draw_label(layer, &label_left_dsc, &bat_area);
#endif // CONFIG_NICE_EPAPER_ON
    if (state->charging) {
